...
export class TreeChart {
  constructor() {
    ...
    Object.keys(attrs).forEach(key => {
      //@ts-ignore
      this[key] = function (_) { //eslint-disable-line
  ...
  render() {
    ...
    d3.select(window).on(`resize.${attrs.id}`, () => {
      const containerRect = container.node().getBoundingClientRect() //eslint-disable-line
      //  if (containerRect.width > 0) attrs.svgWidth = containerRect.width;
      //	main();
    })
  ...
  setDropShadowId(d) {
    ...
    if (typeof DOM != 'undefined') {
      //@ts-ignore
      id = DOM.uid(d.id).id //eslint-disable-line
    }
  ...
  update({ x0, y0, x, y }) {
    ...
    const patternImages = patterns.patternify({ //eslint-disable-line
        tag: 'image', //eslint-disable-line
        selector: 'pattern-image', //eslint-disable-line
        data: d => [d] //eslint-disable-line
      }) //eslint-disable-line
      .attr('x', 0)
      .attr('y', 0)
    ...
    if (typeof attrs.data.nodeIcon !== 'undefined') {
      ...
      nodeEnter
        ...
        .text(({ data }) => data.nodeIcon.icon)
        .attr(
          'style',
          ({ data }) =>
            `font: normal normal normal ${data.nodeIcon.size}px/1 '${attrs.fontFamily}'`
        )
    ...
    nodeEnter
      ...
      .attr(
        ...
        ({ height, data }) =>
          height / 2 -
          (typeof data.nodeIcon === 'undefined' ? 20 : data.nodeIcon.size) -
          5
      )
      .text(({ data }) => `${data.totalSubordinates} ${attrs.subordinates}`)
    ...
    nodeEnter
      ...
      .attr(
        ...
        ({ width, data }) =>
          -width / 2 +
          10 +
          (typeof data.nodeIcon === 'undefined' ? -3 : data.nodeIcon.size)
      )
      .attr('y', ({ height }) => height / 2 - 10)
      .text(({ data }) => `${data.directSubordinates} ${attrs.direct}`)
      .attr('fill', attrs.nodeTextFill)
      .attr('font-weight', 'bold')
      .attr('style', d => (d.children ? null : 'display: none'))
    ...
    nodeUpdate
      ...
      .attr('font-size', ({ children }) => (children ? 40 : 26))
